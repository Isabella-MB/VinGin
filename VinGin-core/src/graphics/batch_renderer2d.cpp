...
namespace vingin
{
	namespace graphics
	{
		...
		void BatchRenderer2D::submit(const Renderable2D *renderable)
		{
			...
			int r = color.x * 255.0f, g = color.y * 255.0f, b = color.z * 255.0f, a = color.w * 255.0f;
			unsigned int c = (a << 24) | (b << 16) | (g << 8) | (r);

			m_VertexData->vertex = position;
			m_VertexData->color = c;
			m_VertexData++;

			m_VertexData->vertex = math::vec3(position.x, position.y + size.y, position.z);
			m_VertexData->color = c;
			m_VertexData++;

			m_VertexData->vertex = math::vec3(position.x + size.x, position.y + size.y, position.z);
			m_VertexData->color = c;
			m_VertexData++;

			m_VertexData->vertex = math::vec3(position.x + size.x, position.y, position.z);
			m_VertexData->color = c;
			m_VertexData++;
